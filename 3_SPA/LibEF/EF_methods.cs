...
namespace LibEF
{
    internal class EF_methods
    {
        ...
        {
            var query = context.Requisicaos.Include(r => r.Obras).AsQueryable();

            if (startDate.HasValue)
                query = query.Where(r => r.DataLevantamento >= startDate);
            if (endDate.HasValue)
                query = query.Where(r => r.DataLevantamento <= endDate);
            var result = query.GroupBy(r => new { r.Obras.NomeObra, r.PkObra })
                    .Select(g => new
                    {
        ...
        public List<Requisicaos> GetRequisicoesLeitor(int PkLeitor)
        {
