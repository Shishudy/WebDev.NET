...
///// this still needs to be tested, validated and optimized
///// this is a draft -> no name checks, no syntax checks
///// this is a draft /////

//// SINCE its admin only
/// tables are passed as they are, no need to id->name conversion
...
public bool	sp_leitor_reactivate (int pk_leitor)
{
	using (var transaction = context.Database.BeginTransaction())
	{
		try
		{
			var leitor = context.Leitors.FirstOrDefault(l => l.PkLeitor == pk_leitor);
			if (leitor == null)
				throw new Exception("leitor not found");
			if (leitor.Stat != "suspended")
	...
	using (var transaction = context.Database.BeginTransaction())
	{
		try
		{
			...
			var requesitions = context.Requisicaos
				.Where(current_date - r.DataLevantamento > date_since);
				.GroupBy(r => r.PkLeitor)
				.Select(g =>g.Key);
			requesitions = requesitions.Where(r => r.Stat != "borrowed");
			foreach (var leitor in leitors)
				sp_del_leitor(leitor.PkLeitor);
			transaction.Commit();
			return true;
		}
		catch (Exception)
	...
	using (var transaction = context.Database.BeginTransaction())
	{
		try
		{
            ...
            var historyEntries = from l in leitor
					join r in context.Requisicaos on l.PkLeitor equals r.PkLeitor
...
public List<RequisicaoStatus> requesicao_status(int pk_leitor, int? pk_nucleo = null)
{
	var leitoresFiltradas = from l in context.Leitors
				where l.PkLeitor == pk_leitor
	var requisicoesFiltradas = from l in context.requesitions
    ...
    foreach (var r in results) //TODO: Botle neck here
    {
